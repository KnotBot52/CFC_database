...
CATEGORY = "Cosmetics"
SEARCH_URL = "https://www.amazon.com/s?k=vegan+cosmetics"
OUTPUT_DIR = os.path.join("data", "products", "cosmetics")

PROMPT_INSTRUCTION = """
Act as an expert vegan and ethical supply chain researcher.
Evaluate the following cosmetic product's materials/ingredients for any animal derivatives.
Cross-reference the Brand Name against known parent company testing policies (e.g., Leaping Bunny, PETA, or gray areas).
Assess supply chain labor practices based on your training data context.
"""
...
async def scrape_product(page: Page, url: str):
    ...
    if not title:
        try:
            ...
            with open("product_error_dump.html", "w", encoding="utf-8") as f:
                f.write(html)
        except:
            pass
...
async def main():
    async with async_playwright() as p:
        ...
        max_pages = 2  # limit to 2 pages for demo purposes
        page_num = 1
        ...
        while current_url and page_num <= max_pages:
            ...
            if page_num == 1:
                html = await page.content()
                with open("amazon_success_dump.html", "w", encoding="utf-8") as f:
                    f.write(html)
            ...
            for url in product_urls:
                ...
                try:
                    ...
                    if not data['title']:
                        print(f"Skipping {url} because Title was empty (DOM timeout or CAPTCHA).")
                        continue
                    ...
                    await asyncio.sleep(2)  # pause between products
