...
CATEGORY = "Clothing"
SEARCH_URL = "https://www.amazon.com/s?k=vegan+clothing"
OUTPUT_DIR = os.path.join("data", "products", "clothing")

PROMPT_INSTRUCTION = """
Act as an expert vegan and ethical supply chain researcher.
Evaluate the following clothing product's materials/fabric for any animal derivatives (e.g., leather, wool, silk, suede, cashmere).
Cross-reference the Brand Name against known parent company policies regarding animal skins and cruelty.
Assess supply chain labor practices based on your training data context (e.g., sweatshop labor, fast fashion, fair trade).
"""
...
async def scrape_product(page: Page, url: str):
    ...
    ingredients = ""  # We reuse "ingredients" to store "materials"
    try:
        # Clothing materials are often in the product description, bullets or Product details
        ...
        detail_bullets = page.locator('#detailBullets_feature_div')
        if await detail_bullets.count() > 0:
            ingredients += "\n" + await detail_bullets.inner_text()
    except:
        pass
